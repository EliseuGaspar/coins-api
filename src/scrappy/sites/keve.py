...
class Keve(ISites):
    ...
    __object_id: str = "Scrapper_BancoKeve"
    __url: str = "https://www.keve.ao/cambios?slug=exchanges"
        ...
        try:
            exchange = (
                await self.__exchanges_controller.find_exchanges_for_bank_name(
                    bank_name="Banco Keve", verify_only=True
                )
            )
            if not exchange:
                exchange_entitie = ExchangeBase(
                    bank_name="Banco Keve", exchanges_values=self.__datas_json
                )
