...
class BFA(ISites):
    ...
    __object_id: str = "Scrapper_BancoBFA"
    __url: str = "https://www.bfa.ao/pt/particulares/pesquisa-de-cambios/"
    __table_striped_class_name: str = ".table"
        ...
        try:
            exchange = (
                await self.__exchanges_controller.find_exchanges_for_bank_name(
                    bank_name="BFA", verify_only=True
                )
            )
            if not exchange:
                exchange_entitie = ExchangeBase(
                    bank_name="BFA", exchanges_values=self.__datas_json
                )
