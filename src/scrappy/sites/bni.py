...
class BNI(ISites):
    ...
    __object_id: str = "Scrapper_BancoBNI"
    __url: str = "https://www.bni.ao/pt/taxas"
            ...
            if exchange_table_element:
                ...
                if all_tr and isinstance(all_tr, list):
                    for tr in all_tr:
                        ...
                        sale: ElementHandle = await tr.query_selector("td.sell")
                        buy: ElementHandle = await tr.query_selector("td.buy")
        ...
        try:
            exchange = (
                await self.__exchanges_controller.find_exchanges_for_bank_name(
                    bank_name="BNI", verify_only=True
                )
            )
            if not exchange:
                exchange_entitie = ExchangeBase(
                    bank_name="BNI", exchanges_values=self.__datas_json
                )
